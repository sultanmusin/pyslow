...
class Detector:
    def __init__(self, config):
        ...
        for id, cfg in config.systemModules.items():
            self.systemModules[id] = HVsysSystemModule(cfg)
    ...
    async def poll_module_important(self, id, poll_cb):   # 
        ...
        for part in module.parts:
            addr = module.addr[part]
            part_type = HVsys.catalogus[part]
            await self.poll_part_important(module.systemModuleId, part_type, addr, poll_cb)
    ...
    async def poll_part_important(self, sys_mod_id, part_type, address, poll_cb):
        ...
        all_capabilities = part_type.capabilities

        for cap in all_capabilities:
            if cap in important_hv_capabilities:
                command = Message(Message.READ_SHORT, address, part_type, cap, 0)
                await self.add_task(sys_mod_id, command, partial(poll_cb, cap))

    async def add_task(self, sys_mod_id, command, cb):
        await self.systemModules[sys_mod_id].add_task(command, cb)
